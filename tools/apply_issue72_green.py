...
constants_marker = "_API_RESPONSE_TIMEOUT_MS = 10_000\n"
constants = (
...
'''
if "def parse_allianz_fund_options" not in text:
    text = text.replace(
        parser_marker,
        "\n" + parser_code + parser_marker.lstrip("\n"),
        1,
    )
...
'''
if "async def scrape_allianz_playwright" not in text:
    text = text.replace(
        handler_marker,
        "\n" + handler_code + handler_marker.lstrip("\n"),
        1,
    )

dispatch_marker = '''    if method == "playwright" and issuer == "Mega":
        return await scrape_mega_playwright(etf_code, page)
'''
dispatch_code = '''    if method == "playwright" and issuer == "Allianz":
        return await scrape_allianz_playwright(etf_code, page)
'''
if dispatch_code not in text:
    text = text.replace(
        dispatch_marker,
        dispatch_code + dispatch_marker,
        1,
    )
