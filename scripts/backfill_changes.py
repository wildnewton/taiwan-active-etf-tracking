...
def backfill_changes(from_date=None, to_date=None, regenerate_signals=False):
    """Recompute historical holding changes and optionally manager signals.
...
def _parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Backfill ETF holding changes and optionally manager signals."
    )
    parser.add_argument(
        "--db",
        dest="db_path",
        default=str(db.DEFAULT_DB_PATH),
        help="SQLite database path. Defaults to data/etf_holdings.sqlite3.",
    )
