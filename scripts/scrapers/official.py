...
_ALLIANZ_ANTIFORGERY_PATH = "/webapi/api/AntiForgery/GetAntiForgeryToken"
...
def parse_allianz_api(
    ...
    *,
    ...
    if expected_date is not None and date != expected_date.strftime("%Y/%m/%d"):
        raise ValueError(
            f"Allianz holdings date mismatch: expected {expected_date.isoformat()}, "
            f"got {date}"
        )
...
def scrape_allianz_api(etf_code: str, target_date: date) -> dict:
    etf_code = etf_code.upper()
    config = get_official_config(etf_code)
    source_url = config["url"]
    antiforgery_url = urljoin(source_url, _ALLIANZ_ANTIFORGERY_PATH)
    options_url = urljoin(source_url, _ALLIANZ_FUND_OPTIONS_PATH)
    trade_url = urljoin(source_url, _ALLIANZ_TRADE_INFO_PATH)

    try:
        with requests.Session() as session:
            antiforgery_response = session.get(antiforgery_url, timeout=30)
            antiforgery_response.raise_for_status()
            antiforgery_data = antiforgery_response.json()
            token = (
                str(antiforgery_data.get("token") or "").strip()
                if isinstance(antiforgery_data, dict)
                else ""
            )
            if not token:
                raise ValueError("Allianz antiforgery token missing")
            headers = {"X-XSRF-TOKEN": token}

            options_response = session.post(
                options_url,
                json={"TypeID": -1, "IsAddAllOption": False},
                headers=headers,
                timeout=30,
            )
            options_response.raise_for_status()
            expected_fund_no = parse_allianz_fund_options(
                options_response.text,
                etf_code,
            )

            taipei_midnight = datetime.combine(
                target_date,
                time.min,
                tzinfo=_TAIPEI_TIMEZONE,
            )
            utc_midnight = taipei_midnight.astimezone(timezone.utc)
            request_date = utc_midnight.isoformat(
                timespec="milliseconds"
            ).replace("+00:00", "Z")
            trade_response = session.post(
                trade_url,
                json={"Date": request_date, "FundNo": expected_fund_no},
                headers=headers,
                timeout=30,
            )
