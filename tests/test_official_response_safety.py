...
@pytest.mark.parametrize(
    ("predicate", "valid_url", "expected_method"),
    [
...
def test_api_response_predicates_fail_closed_without_string_method(
    ...
    missing_request = _Response(valid_url, method=expected_method)
    missing_request.request = None

    missing_method = _Response(valid_url, method=expected_method)
    missing_method.request.method = None

    non_string_method = _Response(valid_url, method=expected_method)
    non_string_method.request.method = Mock()

    assert predicate(missing_request) is False
    assert predicate(missing_method) is False
    assert predicate(non_string_method) is False
